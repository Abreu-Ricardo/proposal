...
static __always_inline int verifica_ip(struct __sk_buff *ctx){
	...
	if (bpf_ntohs(eth->h_proto) == ETH_P_IP){	
        ...
        if(data + sizeof(struct ethhdr) + sizeof(struct iphdr) <= data_end){
			if ( bpf_ntohl(iph->daddr) == IP_N4v7 )
		...
		if(data + sizeof(struct ethhdr) + sizeof(struct iphdr) <= data_end){
           // struct icmphdr *icmp = data + sizeof(struct iphdr);
            ...
            //if (data + sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct icmphdr) <= data_end ){ 
                ...
                // MAC DEST veth5 do Container N3
                // 6e:ae:7d:19:ee:7f
                eth->h_dest[0] = 0x6e;
                eth->h_dest[1] = 0xae;
                eth->h_dest[2] = 0x7d;
				...
				iph->saddr = bpf_htonl(IP_N4v6); 
                iph->daddr = bpf_htonl(IP_N3v5);
...
// CODE TC ingress
SEC("tc")
int tc_in_redir8_6(struct __sk_buff *ctx ){
	__u32 key = 0;
    ...
    //if ( protocolo  == 1 ){ // Filtra pacotes
        if ( ip_ret == 0  /*&& protocolo == 17*/ ){ // Se pacote do IP do container2
        ...
        return bpf_redirect( /*ctx->ingress_ifindex*/ 2 , /*BPF_F_INGRESS*/ 0 ); 
    }
