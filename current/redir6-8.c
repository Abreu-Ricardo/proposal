...
static __always_inline int verifica_ip(struct __sk_buff *ctx){
	...
	if (bpf_ntohs(eth->h_proto) == ETH_P_IP){	
        ...
        if(data + sizeof(struct ethhdr) + sizeof(struct iphdr) <= data_end){
			if ( bpf_ntohl(iph->daddr) == IP_N4v6 )
		...
		if(data + sizeof(struct ethhdr) + sizeof(struct iphdr) <= data_end){
           // struct icmphdr *icmp = data + sizeof(struct iphdr);
            ...
            //if (data + sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct icmphdr) <= data_end ){ 
                ...
                // MAC DEST veth8 do Container Servidor
                // 52:a7:4f:12:71:25 
                eth->h_dest[0] = 0x52;
                eth->h_dest[1] = 0xa7;
                eth->h_dest[2] = 0x4f;
				...
				iph->saddr = bpf_htonl(IP_N4v7); 
                iph->daddr = bpf_htonl(IP_Sv8);
...
// CODE TC ingress
SEC("tc")
int tc_in_redir6_8(struct __sk_buff *ctx ){
	__u32 key = 0;
